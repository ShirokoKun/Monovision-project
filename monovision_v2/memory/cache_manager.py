...
class CacheManager:
    ...
    async def cache_embedding(self, image_hash: str, embedding: List[float], keywords: List[str]):
        ...
        try:
            ...
            if not embedding or len(embedding) == 0:
                logger.warning(f"⚠️ Skipping cache for {image_hash}: empty embedding")
                return
    ...
    async def find_similar_embeddings(self, query_embedding: List[float], similarity_threshold: float = 0.85) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not query_embedding or len(query_embedding) == 0:
                logger.warning("⚠️ Query embedding is empty, skipping similarity search")
                return None
            
            # Get all embeddings from database
            cursor = self.db_conn.execute(
                "SELECT image_hash, embedding, keywords, timestamp FROM embeddings_cache WHERE timestamp > ?",
                (time.time() - 24 * 60 * 60,)  # Only check embeddings from last 24 hours
            )
            ...
            for image_hash, embedding_blob, keywords_json, timestamp in rows:
                try:
                    cached_embedding = np.array(pickle.loads(embedding_blob))
                    
                    # Check if cached embedding is valid
                    if cached_embedding.size == 0:
                        logger.warning(f"⚠️ Cached embedding for {image_hash} is empty, skipping")
                        continue
                    
                    # Check if embeddings have compatible shapes
                    if query_embedding.shape != cached_embedding.shape:
                        logger.warning(f"⚠️ Embedding shape mismatch for {image_hash}: {query_embedding.shape} vs {cached_embedding.shape}")
                        continue
                    ...
                    logger.error(f"❌ Error comparing embedding for {image_hash}: {e}")
                    continue
            
            if best_match:
                logger.info(f"🎯 Found similar image: {best_match['image_hash']} (similarity: {best_match['similarity']:.3f})")
                return best_match
